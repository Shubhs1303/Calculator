...
const startAudio = new Audio("https://github.com/praxeds/theodinproject-calculator/blob/main/assets/audio/start.mp3?raw=true");
const clickSound = new Audio("https://github.com/praxeds/theodinproject-calculator/blob/main/assets/audio/click.mp3?raw=true");
  ...
  switch (button) {
    ...
    case "÷":
      ...
      if (calcOutput.innerText.length > 0 && displayNumber2 === undefined) {
        ...
        keyOperator = document.querySelector(`div[data-key="${button}"]`)
          .classList[1];
    ...
    if (typeof operatorSymbol == "string" && displayNumber1 != "") {
      ...
      if (displayNumber2 !== undefined) {
        switch (true) {
          ...
          case button == ".":
            if (
              calcOutput.innerText.includes(".") == false &&
              calcOutput.innerText.length > 0
            ) {
              calcOutput.innerHTML += button;
            }
            break;
